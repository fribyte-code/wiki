...
description = "Explanation of how the Student Radio's image storage service is set up"
...
[extra]
lang = "en"
translation = "docs/produksjon/bilder-srib-no.md"
+++

### Brief explanation

This is a service that the student radio "uses" to store its images. It is a
service called [PhotoPrism](https://github.com/photoprism/photoprism).
...
What is worth noting here is this:
...
We import a local folder on the VM into the Docker container. This is the same
practice as [podcast.srib.no](/docs/produksjon/podcast-srib-no)

The local folder is also a mounted folder from the Student Radio's NAS. We use
the following line in `fstab` to mount the folder from the NAS.

```
//158.37.6.118/Bilder/  /home/fribyte/photoprism/srib-bilder    cifs    vers=3.0,username=fribyte,password=aiaiaiai,noexec  0 0
```

The username and password are for the user on the Student Radio's NAS. The file
share requires the Student Radio to approve that we can connect to their NAS,
through the software on their NAS. You can find the password in
[bitwarden.fribyte.no](https://bitwarden.fribyte.no)

This is for both reading and writing, of course. Files that are located in that
folder on the NAS are shown in `bilder.srib.no`, and all images uploaded to
`bilder.srib.no` end up on their NAS.
...
Originally, there was a problem where the size of the image being uploaded was
too large. This was because `client_max_body_size` in nginx-proxy's `nginx.conf`
was set to 1MB, while Azuracast's `client_max_body_size` was set to 50MB. So we
also set nginx-proxy's `client_max_body_size` to 50MB.
