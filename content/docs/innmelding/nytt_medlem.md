...
5. Legg til ny epost addresse i tekstfeltet for Local Recipients
6. Lagre endringer
7. Enda mer profitt
...
[HeadScale](https://headscale.net) er en open source versjon av TailScale. Det
tillater oss å SSH'e direkte til maskintjenere i Andeby, uten å måtte gå gjennom
bestemor.

1. Det nye medlemmet må laste ned Tailscale client.
   [Metoden avhenger av systemet](https://headscale.net/docs/installation/).
...
6. An existing friByte member need to do the following:
   - Connect to HeadScale server
   - SSH to headscale virtual machine with `ssh fribyte@headscale`
...
7. The new member should now get `Success` message in the Powershell window.
...
6. An existing friByte member need to do the following: a. Connect to HeadScale
   server b. SSH to headscale virtual machine with `ssh fribyte@headscale` c.
   Run `sudo ./join.sh <USERNAME> <mkey:LONGKEY>`
   - Replace `<USERNAME>` with the new member's chosen username.
...
1. Koble til Headscale VPN.
2. Logg inn med admin-bruker
   [på denne nettsiden](https://proxmox:8006/#v1:0:18:4:::::8::14).
3. Trykk på "Datacenter"
4. Under "Permissions", trykk på "Users"
5. Trykk "Add" og fyll inn medlemmets info.
6. Profitt
7. Nytt medlem kan nå logge inn på [https://proxmox:8006](https://proxmox:8006),
   så lenge de er tilkoblet headscale VPN.
...
## Andeby (Optional, man trenger ikke dette når man har HeadScale)
...
`ssh-keygen -t ed25519 -C "your_email@example.com"` for å generere en
SSH-nøkkel. Få mer info:
...
2. Hente .pub key fra det nye medlemmet, skriv ut public key med
   `cat .ssh/*.pub` og sende det til aktivt medlem i friByte chat.

3. Neste steg må et aktivt medlem i friByte som har tilgang til Andeby gjøre.

4. Koble til bestemor via `ssh root@andeby.fribyte.no`. Dette er gatewayen til
   resten av våre kjære maskiner.

5. Åpne authorized_keys i din favoritt kode-redaktør, nano for eksempel:
